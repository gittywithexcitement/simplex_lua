...
--
-- 8) Fatal error if the same recipe is used multiple times in the objective.
--------------------------------------------------------------------------------
local function optimize(lp, recipe_name, weight, direction)
  if lp.objectives[recipe_name] then
    fatal_error(("Objective for recipe '%s' is already defined."):format(recipe_name))
  end
